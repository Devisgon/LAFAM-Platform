-- supabase\migrations\20260620134553_fix_mark_payment_paid_atomic_gateway_arguments.sql
--
-- Problem:
-- The NestJS PaymentRepository calls mark_payment_paid_atomic with:
-- - p_gateway_payment_id
-- - p_gateway_invoice_id
-- - p_next_status
--
-- The database currently exposes the older function signature without those
-- arguments, causing PostgREST PGRST202 during hosted payment callback settlement.
--
-- Correct behavior:
-- The paid mutation RPC must accept the backend's current argument contract,
-- preserve gateway identifiers, mark the payment as paid atomically, confirm
-- payable booking/private booking targets, and credit wallet top-up payments.

begin;

drop function if exists public.mark_payment_paid_atomic(
  uuid,
  text,
  text,
  text,
...
begin
  ...
  set
    ...
    gateway_reference = coalesce(v_provider_reference, gateway_reference),
    gateway_payment_id = coalesce(v_gateway_payment_id, gateway_payment_id),
    gateway_invoice_id = coalesce(v_gateway_invoice_id, gateway_invoice_id),
