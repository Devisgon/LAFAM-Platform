...
import { bookingDateLabel, bookingTitle, DONUT_CIRCUMFERENCE, formatAxisMoney, formatMoney, formatPaymentStatus, formatShortDate, statusTone, type ChartPoint } from "../../utils/dashboardFormatters";
...
export function DashboardContent({ dashboard }: { dashboard: AnalyticsDashboard }) {
...
export function RevenueOverviewChart({ data }: { data: AnalyticsRevenueWeekPoint[] }) {
...
export function PaymentSummaryChart({ summary }: { summary: AnalyticsPaymentSummary }) {
  const segments = [
...
export function RecentBookings({ bookings }: { bookings: AnalyticsBookingListItem[] }) {
  return (
...
export function TopServices({ services }: { services: AnalyticsTopServiceItem[] }) {
