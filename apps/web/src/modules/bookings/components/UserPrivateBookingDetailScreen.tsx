...
import { useUserPrivateBookingDetail } from "../hooks/useUserBookings";
import type {
  UserPrivateBooking,
...
function customerName(booking: UserPrivateBooking): string {
  return (
...
export function UserPrivateBookingDetailScreen({
  privateBookingId,
}: {
  privateBookingId: string;
}) {
  const detail = useUserPrivateBookingDetail(privateBookingId);
  ...
  return (
    <div className="grid gap-6 text-txt-primary">
        ...
        <div className="mt-5 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-bold">
              {booking?.booking_number ?? "Loading private booking"}
            </h1>
          </div>
          {booking ? (
      ...
      {detail.isLoading ? (
        <LoadingState className="rounded-2xl border border-background-secondary bg-card-bg-primary p-8" label="Loading private booking details" />
      ) : null}

      {booking ? <PrivateBookingDetailContent booking={booking} /> : null}
    </div>
  );
}

function PrivateBookingDetailContent({
  booking,
}: {
  booking: UserPrivateBookingDetail;
}) {
  return (
    <div className="grid gap-5">
      ...
      <DetailSection
        ...
        items={[
          ...
          ["Seat hold expires", formatDateTime(booking.seat_hold_expires_at)],
          ["Cancellation reason", booking.cancellation_reason],
          ["Rescheduled at", formatDateTime(booking.rescheduled_at)],
          ["Realtime version", booking.realtime_version],
        ]}
      ...
      <DetailSection
        ...
        items={[
          ...
          ["Customer email", booking.customer?.email ?? null],
          ["Customer phone", booking.customer?.phone ?? null],
        ]}
        title="People"
      />
